...
{
    internal class GameWindowHooker : IGameWindowHooker, IEnableLogger
    {
        ...
        private readonly Subject<WindowSizeDelta> _gameSizeChangedSubj = new();
        public IObservable<WindowSizeDelta> GameSizeUpdated => _gameSizeChangedSubj;
        ...
        public int SetupGameWindowHook(Process process)
        {
            _gameProc = process;
            ...
            State.UpdateGameRealWindowHandle(_gameHwnd);
            ...
            // EnableRaisingEvents would create a ThreadPool Wait thread
        ...
        private const int GoodWindowWidth = 160;

        private const int GoodWindowHeight = 120;

        private static nint CurrentWindowHandle(Process proc, bool activeGame = true)
        {
                ...
                while (spendTime.Elapsed.TotalMilliseconds < WaitGameStartTimeout)
                {
                    ...
                    foreach (var handle in handles)
                    {
                        ...
                        if (clientRect.bottom > GoodWindowHeight &&
                            clientRect.right > GoodWindowWidth)
                        {
                            LogHost.Default.Info($"Set new handle 0x{handle.DangerousGetHandle():X8}");
                        ...
                        if (hWnd == _gameHwnd)
                        {
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    this.Log().Info("Game window show - recreate");
                                    UpdateLocation();
                                }
                            }
                ...
                case EventObjectShow:
                    {
                        ...
                        {
                            _ViewOperationSubj.OnNext(ViewOperation.Show);
                            this.Log().Info("Game window show");
                            UpdateLocation();
                        }
                    }
                    break;
                case EventObjectHide:
                    {
                        if (_gameProc.MainWindowHandle != hWnd && hWnd == _gameHwnd)
                        {
                            ...
                            this.Log().Info("Game window hide");
                        }
                    }
                    break;
        ...
        private WindowPosition UpdateLocation()
        {
            ...
            _gameSizeChangedSubj.OnNext(new WindowSizeDelta(left - _lastPos.Left, top - _lastPos.Top));
